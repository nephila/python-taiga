...
class TestAuthApp(unittest.TestCase):
    ...
    def test_auth_not_success(self, requests):
        ...
        api = TaigaAPI(host='host')
        self.assertRaises(
            taiga.exceptions.TaigaRestException, api.auth_app, 'valid-app-id', 'valid-app-secret',
    ...
    def test_auth_connection_error(self, requests_post):
        ...
        api = TaigaAPI(host='host')
        self.assertRaises(
            taiga.exceptions.TaigaRestException, api.auth_app, 'valid-app-id', 'valid-app-pass',
            'valid-auth-code', 'valid-state'
        )
